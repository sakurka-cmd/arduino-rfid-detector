# Arduino RFID Card Cloner v3.7

Устройство на базе Arduino Uno для определения типа RFID-карты и клонирования MIFARE Classic на перезаписываемые карты (CUID/FUID). Поддерживает 3 режима работы: DETECT (определение типа), READ (чтение дампа), WRITE (запись на CUID/FUID). v3.7 — рабочая версия с direct write после select без reSelect.
...
|-----------|----------|------------|
| Перезаписываемая карта (CUID/FUID) | Пустая MIFARE Classic 1K с перезаписываемым UID | AliExpress: «CUID card» или «FUID card», ~$0.5-1/шт |
...
5. Поднесите пустую перезаписываемую карту (CUID/FUID) → Arduino запишет дамп
6. Готово! Карта теперь является копией оригинала
...
|-----|---------|--------|-------------|------------|
| MIFARE Classic 1K | 13,56 МГц | Полное (все 16 секторов) | Да (CUID/FUID) | Домофонные ключи Цифрал |
| MIFARE Classic 4K | 13,56 МГц | Частичное (16 из 40) | Частично (CUID/FUID 4K) | 40 секторов не влезают в 2 КБ RAM Arduino |
| MIFARE Mini | 13,56 МГц | Полное (5 секторов) | Да (CUID/FUID) | Компактная версия |
...
- **PICC_HaltA() вызывает зависание Arduino** — RC522 не генерирует IRQ при HaltA, и Arduino навсегда зависает в ожидании. **Никогда не использовать.**
- **Вместо HaltA: `resetCard()`** — используется `PCD_AntennaOff()` / `PCD_AntennaOn()` для физического сброса карты (аналог HALT).
- **ДНР USB SER (клон CH340) не поддерживает DTR auto-reset** — для прошивки нужен физический RESET + `avrdude`
- **Блок 0 (UID) factory-locked на стандартных MIFARE** — для записи UID нужна перезаписываемая карта (CUID/FUID)
- **Сектора секторного трейлера (block 3 каждого сектора) НЕЛЬЗЯ перезаписывать при клонировании** — это вызывает блокировку доступа к сектору
- **reSelect (antenna toggle) между попытками записи приводит к потере карты** — писать нужно сразу после первичного Select без повторного выбора
...
| Версия | Описание |
|--------|----------|
| v1.1 | Детектор типов карт (только DETECT) |
| v2.0 | Добавлены READ и WRITE, брутфорс ключей |
| v2.2 | Устранён баг PICC_HaltA (зависание Arduino) |
| v3.0-v3.6 | Отладка: magic UID, no-CRC, raw register access |
| v3.7 | **Рабочая версия!** Direct write после select без reSelect |
...
| `rfid_cloner.ino` | v3.7 — рабочий клонер MIFARE Classic (DETECT/READ/WRITE) |
| `rfid_cloner_v22.ino.bak` | v2.2 — резервная копия |
